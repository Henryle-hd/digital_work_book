...
// import ParticleComponent from '../components/exampleQuestionForm';
      ...
      {loading ? (
        <div>
          ...
          <div className="max-w-2xl mx-auto border-2 border-sky-200 rounded-3xl p-10 bg-white shadow-2xl hover:transform hover:scale-105 transition-all duration-500 backdrop-blur-sm my-8">
            <h1 className="text-xl font-bold mb-6 text-sky-900 text-center">📝 Welcome to Your Personal Workbook!</h1>
        ...
        </div>      ) : (        <div className="max-w-3xl mx-auto">          <h1 className="text-5xl font-bold text-sky-900 text-center mb-12">
            Personal Development Workbook
          </h1>
        
          <form className="space-y-8">
            {questions.map((q) => (
              <div key={q.question} className="bg-white shadow-2xl rounded-3xl p-8 hover:transform hover:scale-105 transition-all duration-500">
                <label className="block text-xl font-semibold text-sky-900 mb-4">
                  {q.question}
                </label>
                {q.type === 'textarea' ? (
                  <textarea
                    ...
                    name={q.id}
                    rows={4}
                    className="w-full rounded-xl border border-sky-200 shadow-sm p-4 focus:ring-sky-500 focus:border-sky-500 text-gray-700"
